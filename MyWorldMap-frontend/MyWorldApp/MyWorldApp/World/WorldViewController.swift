...
class WorldViewController: UIViewController {
    ...
    private lazy var mapView = MapView(
            countryColorViewModel: userCountryColorsModel,
            onLocationSelected: {
                self.navigateToDetailView()
            }
        )

    private lazy var hostingController = UIHostingController(rootView: mapView)
    ...
    func updateWorldViewUI(with countryData: String) {
        // 지도 위치 바꿔버리기
        
        if let countryCode = countryCodes[countryData] {
            hostingController.rootView.mapViewModel.zoomToCountry(countryCode)
        }
...
extension WorldViewController: UISearchBarDelegate {
    // 검색 시작될 때 호출되는 메서드
